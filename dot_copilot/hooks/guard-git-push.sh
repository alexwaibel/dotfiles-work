...
if [[ ${#ARGS[@]} -lt 2 ]]; then
  deny "Bare 'git push' without an explicit branch is not allowed. Specify the branch, e.g. git push origin users/alwaibel/agent/<id>-<slug>"
fi

ALLOWED_PREFIX="users/alwaibel/agent/"
...
for ((i=1; i<${#ARGS[@]}; i++)); do
  ...
  if [[ "$ref" == *:* ]]; then
    ...
    if [[ "$dest" != "${ALLOWED_PREFIX}"* ]]; then
      deny "Refspec destination '$dest' is not under '${ALLOWED_PREFIX}*'. Pushes are only allowed to users/alwaibel/agent/* branches."
    fi
  else
    if [[ "$ref" != "${ALLOWED_PREFIX}"* ]]; then
      deny "Branch '$ref' is not under '${ALLOWED_PREFIX}*'. Pushes are only allowed to users/alwaibel/agent/* branches."
    fi
  fi
done
