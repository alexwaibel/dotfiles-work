...
echo "=== Allowed pushes ==="
expect_allow "valid branch"               "git push origin alwaibel/agent/123-test"
expect_allow "with -u flag"               "git push -u origin alwaibel/agent/456-feat"
expect_allow "with --force flag"          "git push --force origin alwaibel/agent/789-fix"
expect_allow "valid refspec"              "git push origin alwaibel/agent/1-a:alwaibel/agent/1-a"
expect_allow "multiple valid branches"    "git push origin alwaibel/agent/1-a alwaibel/agent/2-b"
...
expect_block "refspec to main"           "git push origin alwaibel/agent/123-test:main"
...
expect_block "chain with &&"            "git push origin alwaibel/agent/1-a && git push origin main"
expect_block "chain with ;"             "git push origin alwaibel/agent/1-a ; git push origin main"
expect_block "chain with ||"            "git push origin alwaibel/agent/1-a || git push origin main"
expect_block "pipe"                      "git push origin alwaibel/agent/1-a | cat"
expect_block "subshell"                  "git push origin \$(echo main)"
expect_block "backticks"                 "git push origin \`echo main\`"
expect_block "one valid one bad"         "git push origin alwaibel/agent/1-a main"
